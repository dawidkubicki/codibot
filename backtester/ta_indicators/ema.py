...
from strategies.ta_indicators.ema import EMAStrategy
...
class EMA:
    ...
    def double_ema(self,
        ...
        for pairs, data in new_data.items():
            ...
            strategy = EMAStrategy(data)
            self.ema_data[pairs] = strategy.double_ema(ema_s, ema_l, technical, shifting)

            if fixed_stop_loss and not trailing_stop_loss and not atr_stop_loss:
                self.ema_data[pairs] = ExitRules(data).fixed_stop_loss(stop_loss)
            elif trailing_stop_loss and not fixed_stop_loss and not atr_stop_loss:
                self.ema_data[pairs] = ExitRules(data).trailing_stop_loss(stop_loss)
            elif atr_stop_loss and not fixed_stop_loss and not trailing_stop_loss:
                self.ema_data[pairs] = ExitRules(data).atr_stop_loss(window=atr_window,
                                                                     stop_loss=stop_loss,
                                                                     ema_calc=ema_calc,
    ...
    def triple_ema(self,
        ...
        for pairs, data in new_data.items():
            ...
            strategy = EMAStrategy(data)
            self.ema_data[pairs] = strategy.triple_ema(ema_s, ema_m, ema_l, technical, shifting)
    ...
    def run_backtest(self):
        if self.ema_data is None:
            ...
            self.backtested_data = self.backtester.test_strategy(self.ema_data)
        ...
        if self.strategy == "double_ema":
            ...
            for asset in assets:
                indexes = self.performance_data[self.performance_data["Asset"] == asset]["cstrategy"].nlargest(
                    3).index.values
                ema_s, ema_l, creturns, cstrategy = [], [], [], []
                for idx in indexes:
                    ema_s.append(int(self.performance_data["EMA_S"].iloc[int(idx)]))
                    ema_l.append(int(self.performance_data["EMA_L"].iloc[int(idx)]))
            ...
            for asset in assets:
                indexes = self.performance_data[self.performance_data["Asset"] == asset]["cstrategy"].nlargest(
                    3).index.values
                ema_s, ema_l, ema_m, creturns, cstrategy = [], [], [], [], []
                for idx in indexes:
                    ema_s.append(int(self.performance_data["EMA_S"].iloc[int(idx)]))
    ...
    def all_combinantion(self, EMA_S_range=None, EMA_M_range=None, EMA_L_range=None, technical=True, shifting=False):
        ...
        if self.strategy == "double_ema":
            ...
            for ema_s, ema_l in tqdm(combinations):
                try:
                    self.double_ema(ema_s, ema_l, technical, shifting)
                    self.run_backtest()
            ...
            for ema_s, ema_m, ema_l in tqdm(combinations):
                try:
                    self.triple_ema(ema_s, ema_m, ema_l, technical=technical, shifting=shifting)
                    self.run_backtest()
                    for pair, result in self.backtested_data.items():
                        comb_pair.append(pair)
                        comb_s.append(ema_s)
